...
DmMotorDriver::DmMotorDriver(uint16_t motor_id, std::string can_interface, uint16_t master_id,
                             DM_Motor_Model motor_model)
                             : can_(SocketCAN::get(can_interface)), motor_model_(motor_model){   
    motor_id_ = motor_id;
...
void DmMotorDriver::MotorDisEna() {
    can_frame tx_frame;
    tx_frame.can_id = motor_id_;  // change according to the mode
    tx_frame.can_dlc = 0x08;
